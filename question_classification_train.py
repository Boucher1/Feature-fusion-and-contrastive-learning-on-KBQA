...
from model import simcse_unsup_loss, simcse_sup_loss, SimcseModel_T5_CNN, Matching_network
...
def parse_args():
	...
	parser.add_argument("--train_file", type=str, default="data/webQSP/hop_classification/contras_data/train.csv")
	parser.add_argument("--dev_file", type=str, default="data/webQSP/hop_classification/contras_data/dev.csv")
	parser.add_argument("--test_file", type=str, default="data/webQSP/hop_classification/contras_data/test.csv")
	...
	parser.add_argument("--train_mode", type=str, default='question_classification', choices=['unsupervise', 'supervise'],
	...
	args.output_path = join(args.output_path, args.train_mode,
							'{}-{}-bsz-{}-lr-{}-drop-{}'.format(args.pretrain_model_path.split('/')[-1], args.dataset, args.batch_size_train, args.lr, args.dropout))
	return args
...
def test(args):
	...
	model = SimcseModel_T5_CNN(pretrained_model=args.pretrain_model_path, pooling=args.pooler, dropout=args.dropout).to(
		args.device)
...
def main(args):
	...
	model = SimcseModel_T5_CNN(pretrained_model=args.pretrain_model_path, pooling=args.pooler, dropout=args.dropout).to(args.device)

	if args.do_train:
		# 加载数据集
		assert args.train_mode in ['supervise', 'unsupervise', 'cons_select', "question_classification"], \
			"train_mode should in ['supervise', 'unsupervise']"
		if args.train_mode in ['supervise', 'cons_select',"question_classification"]:
