...
from model import SimcseModel, simcse_unsup_loss, simcse_sup_loss, SimcseModel_T5, SimcseModel_T5_CNN, SimcseModel_T5_GRU_CNN_Attention
...
def parse_args():
	...
	# 超参数kesai
	parser.add_argument("--kesai", type=float, default=0.5)
	# beam search k
	...
	# parser.add_argument("--train_file", type=str, default="data/webQSP/beam_search/train_shuffle.csv")
	# parser.add_argument("--dev_file", type=str, default="data/webQSP/beam_search/valid_oversampleshuffle.csv")
	# parser.add_argument("--test_file", type=str, default="data/webQSP/beam_search/test_oversampleshuffle.csv")
	# parser.add_argument("--dataset", type=str, default="webQSP")
	parser.add_argument("--train_file", type=str, default="data/complexQA/beam_search/train_shuffle.csv")
	parser.add_argument("--dev_file", type=str, default="data/complexQA/beam_search/valid_oversampleshuffle.csv")
	parser.add_argument("--test_file", type=str, default="data/complexQA/beam_search/test_oversampleshuffle.csv")
	parser.add_argument("--dataset", type=str, default="comQA")
	parser.add_argument("--pretrain_model_path", type=str, default="pretrain_model/Sentence-T5-large")
	...
	args.output_path = join(args.output_path, args.train_mode,
							'{}-GRU-CNN-Attention-beam-search-{}-bsz-{}-lr-{}-drop-{}-top-k'.format(args.pretrain_model_path.split('/')[-1], args.dataset, args.batch_size_train, args.lr, args.dropout))
	return args
		...
		for batch_idx, data in enumerate(tqdm(train_loader)):
			...
			if step % args.eval_step == 0:
				...
				if min_loss > loss:
					...
					# logger.info('higher corrcoef: {} in step {} epoch {}, save model'.format(corrcoef, step, epoch))
					logger.info('lower loss: {} in step {} epoch {}, save model'.format(loss, step, epoch))
		# 每个epoch测试一下
...
def test(args):
	...
	model = SimcseModel_T5_GRU_CNN_Attention(pretrained_model=args.pretrain_model_path, pooling=args.pooler, kesai=args.kesai, dropout=args.dropout).to(
		args.device)
...
def main(args):
	...
	model = SimcseModel_T5_GRU_CNN_Attention(pretrained_model=args.pretrain_model_path, pooling=args.pooler, kesai=args.kesai, dropout=args.dropout).to(args.device)
